...
public class UserResource {
    ...
    public ResponseEntity<UserDao>saveUser(@RequestBody UserDao userDao){
        ...
        return ResponseEntity.created(uri).body(userService.saveUser(userDao));
        ...
        if(authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
            try {
                ...
                String access_token = JWT.create()
                        .withSubject(userDao.getUsername())
                        .withExpiresAt(new Date(System.currentTimeMillis() + 10 * 60 * 1000))
                        .withIssuer(request.getRequestURL().toString())
                        .withClaim("roles", userDao.getRoles().stream().map(Role::getName).collect(Collectors.toList()))
                        .sign(algorithm);
